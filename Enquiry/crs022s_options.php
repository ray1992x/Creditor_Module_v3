		...
		<form id="form" name="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
			<!--<tr>
				...
				<tr>
					...
					<label><input onclick="PayDate()" type="radio" name="CPR_Print" id="CPR_Print" value="Date" <?php if ($_SESSION['ChequePrint']=='Date') { echo 'checked'; } ?> />Payment Date</label><br>
				...
				<tr>
					<td>Payment Date</td>
					<td>
					<label><input class="form-control txt-auto" <?php if(empty($_SESSION['PaymentDateFrom'])) { echo 'disabled';} else { echo 'value="'.$_SESSION['PaymentDateFrom'].'"'; } ?> type="text" name="Payment_Date_From" id="Payment_Date_From" placeholder="From..." /></label>
					<label><input class="form-control txt-auto" <?php if(empty($_SESSION['PaymentDateTo'])) { echo 'disabled';} else { echo 'value="'.$_SESSION['PaymentDateTo'].'"'; } ?> type="text" name="Payment_Date_To" id="Payment_Date_To" placeholder="To..." /></label>
					</td>
				...
				<tr>
					...
					<td>
					<label>
					<select class="form-control txt-auto" <?php if(empty($_SESSION['FiscalPeriodM'])) { echo 'disabled';} ?> STYLE=background:#eeeeee name="Fiscal_Period_M" id="Fiscal_Period_M">
					...
					<label><input class="form-control txt-auto" <?php if(empty($_SESSION['FiscalPeriodY'])) { echo 'disabled';} else { echo 'value="'.$_SESSION['FiscalPeriodY'].'"'; } ?> type="number" name="Fiscal_Period_Y" id="Fiscal_Period_Y" placeholder="Fiscal Year" /></label>
					</td>
				...
				<tr>
					...
					<label><input type="radio" name="CPR_Group" id="CPR_Group" value="DatePaid" <?php if ($_SESSION['GroupBy']=='DatePaid') { echo 'checked'; } ?> />Payment Date</label><br>
				...
				<table>
					<tr>
					<td><button class="btn btn-default" onclick="top.close();"><span class="glyphicon glyphicon-chevron-left"></span> Cancel</button></td>
					<td><button name="Print"  value="Print"  class="btn btn-default" /><span class="glyphicon glyphicon-eye-open"> View</button>
					<td><button name="Print2"  value="Print2"  class="btn btn-default" /><span class="glyphicon glyphicon-eye-open"> All</button>
					</td></tr>
				</table>
		</form> 
	</table>				  
