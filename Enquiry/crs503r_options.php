	...
	<table>
		<form id="form" name="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
			...
			<tr>
				...
				<br>
				<label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;from&nbsp;<input <?php if(empty($_SESSION['PO_From'])) { echo 'disabled';} else { echo 'value="'.$_SESSION['PO_From'].'"'; } ?> type="text" name="PO_From" id="PO_From" /> To <input <?php if(empty($_SESSION['PO_To'])) { echo 'disabled';} else { echo 'value="'.$_SESSION['PO_To'].'"'; } ?> type="text" name="PO_To" id="PO_To" /></label><font color="red"><?php echo $POMLerr2 ;?></font>
				</td>
