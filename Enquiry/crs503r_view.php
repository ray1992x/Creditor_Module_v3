...
$query_cred = "SELECT creditormaster.CreditorName,creditormaster.CreditorCode, POtemp, POAmount, PODate, POType FROM purchase
LEFT JOIN creditormaster
ON purchase.CreditorCode=creditormaster.CreditorCode
WHERE purchase.CreditorCode='".$CR_Code."' ORDER BY creditormaster.CreditorCode";

$query_pod = "SELECT creditormaster.CreditorName,creditormaster.CreditorCode, POtemp, POAmount, PODate, POType FROM purchase
LEFT JOIN creditormaster
ON purchase.CreditorCode=creditormaster.CreditorCode
...
class PDF extends PDF_MYSQL_TABLE
{
	function Header()
	{
		...
		$this->Cell(45, 8, '', 0);
		$this->Cell(100, 8, 'Purchase Order Master Listing', 0,0,'C');
		$this->Ln(20);
...
<?php
	...
	if ($conn->connect_error){
		die("connection failed:" . $conn->connect_error);
			...
			if($PO_PrintOp=='ByCreditor'){
				$sql = $query_cred;//select database
			}else if($PO_PrintOp=='ByPOD'){
				$sql = $query_pod;
			}
