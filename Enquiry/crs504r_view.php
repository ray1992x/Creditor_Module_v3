	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			while($row=mysql_fetch_array($records)){
				if( (!empty($row['POtemp'])) &&
					...
					$exists = "yes";
					if($AllCreditors=='All'){
							$query = "SELECT creditormaster.CreditorCode, creditormaster.CreditorName, POtemp, DATE_FORMAT(PODate,'%b %d, %Y') AS PODate, POType, POAmount FROM purchase
...
<?php
	...
	if ($conn->connect_error){
		die("connection failed:" . $conn->connect_error);
			...
			if($AllCreditors=='All'){
					$sql = "SELECT creditormaster.CreditorCode, creditormaster.CreditorName, POtemp, DATE_FORMAT(PODate,'%b %d, %Y') AS PODate, POType, POAmount FROM purchase
					LEFT JOIN creditormaster
					ON purchase.CreditorCode=creditormaster.CreditorCode
					WHERE ( POtemp NOT IN(
						select PONumber FROM invoice
